...
def create_face_landmarker(min_face_detection_confidence=0.5):
...
def enhance_contrast(bgr_img):
    """CLAHE (contrast-limited adaptive histogram equalization) on the
    lightness channel only. Burn discoloration and medical equipment create
    extreme local contrast/dark regions that can throw off the face detector
    -- this normalizes lighting without distorting hue, giving detection a
    second, cleaner-looking chance."""
    import cv2
...
def _run_detector(bgr_img, detector):
    import mediapipe as mp
    import cv2
    ch, cw = bgr_img.shape[:2]
    ...
    return {i: (l.x * (cw - 1), l.y * (ch - 1)) for i, l in enumerate(lm)}


def detect_landmarks_2d(tif_path, detector, detector_lenient=None):
    """Tries, in order: (1) normal detector on the image as-is, (2) a more
    lenient-confidence detector, (3) the lenient detector on a
    contrast-normalized version of the image. This chain specifically targets
    burn-scarred / equipment-occluded scans that a single strict pass misses.
    Returns (pts2d_all, cw, ch, attempt_used) or (None, cw, ch, None)."""
    import cv2
    ...
    pts2d_all = _run_detector(img, detector)
    if pts2d_all is not None:
        return pts2d_all, cw, ch, "strict"

    if detector_lenient is not None:
        pts2d_all = _run_detector(img, detector_lenient)
        if pts2d_all is not None:
            return pts2d_all, cw, ch, "lenient"

        enhanced = enhance_contrast(img)
        pts2d_all = _run_detector(enhanced, detector_lenient)
        if pts2d_all is not None:
            return pts2d_all, cw, ch, "lenient+enhanced"

    return None, cw, ch, None
...
def process_scan(scan_name, range_path, tif_path, out_json, detector, detector_lenient=None, debug=False):
    pts2d_all, cw, ch, method = detect_landmarks_2d(tif_path, detector, detector_lenient)
    if pts2d_all is None:
        return {"scan": scan_name, "status": "failed", "error": "no face detected (all attempts)"}
    ...
    payload = {
        "scan": scan_name,
        "image_size": [cw, ch],
        "detection_method": method,  # "strict" | "lenient" | "lenient+enhanced" -- non-"strict" = worth spot-checking
    ...
    return {"scan": scan_name, "status": "ok", "method": method,
...
def main():
    ...
    detector = create_face_landmarker(min_face_detection_confidence=0.5)
    detector_lenient = create_face_landmarker(min_face_detection_confidence=0.2)
    try:
        for patient_id, timepoint_id, scan_name, range_path, tif_path, scan_dir in scans:
            ...
            try:
                status = process_scan(scan_name, range_path, tif_path, out_json,
                                       detector, detector_lenient, debug=args.debug)
                if status["status"] == "ok":
                    warn = f"  ⚠ missing stable: {status['missing_stable']}" if status["missing_stable"] else ""
                    tag = "" if status["method"] == "strict" else f"  [{status['method']}]"
                    print(f"✓  {status['n_landmarks']}/{status['n_total']} mesh points{tag}{warn}")
                else:
        ...
        detector.close()
        detector_lenient.close()

    ok       = sum(1 for r in report if r["status"] == "ok")
    non_strict = [r for r in report if r["status"] == "ok" and r.get("method") != "strict"]
    ...
    if non_strict:
        print(f"  Needed fallback detection : {len(non_strict)}  (worth spot-checking debug PNGs)")
        for r in non_strict: print(f"    ⚠  {r['scan']} — {r['method']}")
