...
def run_segmentation(img_rgb: np.ndarray, args) -> np.ndarray:
    ...
    if model_name == "unetpp":
        ...
        mask, _  = predict(model, img_rgb, threshold=args.threshold)

    elif model_name == "segformer":
        from utils.models.segformer import load_model, predict
        model, processor = load_model(args.ckpt)
        mask, _ = predict(model, processor, img_rgb, threshold=args.threshold)
        ...
        mask, _ = predict(model, predictor, img_rgb, threshold=args.threshold)
...
def wrap_and_align(range_path: Path, colored_img: np.ndarray, scan_dir: Path, scan_name: str):
    ...
    doesn't exist yet for this scan, the point cloud is left UNALIGNED rather
    than falling back to the old guess.
    Returns (pcd, pts, colors, aligned: bool).
    """
...
def process_one_scan(scan_dir: Path, args):
    ...
    # Steps 4+5 — 3D
    pcd, pts, colors, aligned = wrap_and_align(range_path, colored_img, scan_dir, scan_name)
