#!/usr/bin/env python3
r"""
step1_compute_region_measurements.py
=======================================
NOTE: this does NOT compute alignment. It READS the alignment already
produced by folder 3 (3.face_alignment/step3_align_landmarks.py) and uses it
to turn each burn region's 2D pixel polygon into real physical measurements.

Reads each scan's:
  - <scan>_burn_polygons.json   (2D polygon in pixel space, from folder 5)
  - <scan>_alignment.json       (rotation/translation to the patient's
                                 per-pose reference frame, from folder 3 --
                                 may not exist for every scan; see below)
...
For each burn region:
  ...
  3. Computes area in real mm² using the cylindrical surface-patch formula:
       area_mm2 = Σ radius_i × dθ × dz   (exact for a cylindrical scanner --
       does NOT require alignment, works even for unaligned scans)
  4. If this scan's alignment.json exists, applies (rotation, translation)
     to move those 3D points into the patient's reference frame; if it
     doesn't exist (landmark detection failed for this scan), area_mm2 is
     still computed, but there is no comparable 3D position -- flagged as
     "no_alignment_file" rather than silently guessing one.
  5. Computes the (aligned, if possible) 3D centroid
...
This file has the same structure as _burn_polygons.json PLUS:
  - area_mm2               (real surface area, always computed)
  - n_points_3d            (how many scanner points fell in this region)
  - aligned_centroid_xyz   ([x, y, z] in reference frame, if aligned)
  - alignment_status       (ok / failed / no_alignment_file)

step3_track_progress.py reads this file preferentially when present, falling
back to the original _burn_polygons.json for any scan that lacks it.
...
USAGE:
  ...
  # one patient, batch over all its scan subfolders
  python step1_compute_region_measurements.py --dataset D:\NahidW\Dataset\face_burn_dataset --patient PAT01

  # single scan
  python step1_compute_region_measurements.py --scandir D:\NahidW\Dataset\face_burn_dataset\PAT01\D00\PAT01_D00_A\
"""
...
sys.path.insert(0, str(Path(__file__).parent.parent / "5.burn_segmentation_3d_pipeline"))
...
def batch(dataset_dir: Path, patient: str) -> dict:
...
def batch_all_patients(dataset_dir: Path):
    """Run batch() for every patient subfolder found directly under dataset_dir."""
    ...
    for pd in patient_dirs:
        ...
        result = batch(dataset_dir, patient)
        total_ok += result["ok"]
        total_skipped += result["skipped"]
        total_failed.extend((f"{patient}/{n}", e) for n, e in result["failed"])
        print()

    print(f"  ALL PATIENTS DONE — {total_ok} processed, "
          f"{total_skipped} skipped, {len(total_failed)} errors")
    for name, err in total_failed:
        print(f"    ✗ {name}: {err}")


def main():
    p = argparse.ArgumentParser(description="Compute real mm² area + aligned centroid per burn region")
    p.add_argument("--dataset", default=None,
                    help="Path to the dataset root (contains one folder per patient)")
    p.add_argument("--patient", default=None,
                    help="Patient ID to process. Omit (with --dataset) to process ALL patients.")
    p.add_argument("--scandir", default=None,
                    help="Path to a single scan folder to process")
    args = p.parse_args()
    ...
    if args.scandir:
        ...
        batch_all_patients(Path(args.dataset))
    else:
        p.error("Provide --scandir, or --dataset (+ optional --patient)")
