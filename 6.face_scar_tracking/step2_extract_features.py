...
USAGE:
  ...
  python step2_extract_features.py --dataset D:\NahidW\Dataset\face_burn_dataset

  # one patient, batch over every scan subfolder found under it
  python step2_extract_features.py --dataset D:\NahidW\Dataset\face_burn_dataset --patient PAT01

  # single scan
  python step2_extract_features.py --polygons D:\NahidW\Dataset\face_burn_dataset\PAT01\D00\PAT01_D00_A\PAT01_D00_A_burn_polygons.json
"""
...
def batch(dataset_dir: Path, patient: str) -> dict:
    ...
    if not files:
        print(f"  No *_burn_polygons.json files found for {patient}")
        return {"ok": 0, "failed": []}

    print(f"  Found {len(files)} scan(s) for {patient}\n")
    ok, failed = 0, []
    for f in files:
        try:
...
def batch_all_patients(dataset_dir: Path):
    ...
    print(f"  ALL PATIENTS DONE — {total_ok} succeeded, {len(total_failed)} failed")
...
def main():
    ...
    p.add_argument("--dataset",  default=None,
                    help="Dataset root (contains one folder per patient). "
                         "Combine with --patient for a single patient, or omit --patient "
                         "to process ALL patients found under it.")
    ...
    if not args.dataset:
        p.error("Provide --polygons for a single file, or --dataset (+ optional --patient) for batch mode")
