...
USAGE:
  # ALL patients in the dataset (every patient folder under --dataset)
  python step3_track_progress.py --dataset D:\NahidW\Dataset\face_burn_dataset

  # one patient — writes one tracking JSON per variant found (A, B, C, D, ...)
  python step3_track_progress.py --dataset D:\NahidW\Dataset\face_burn_dataset --patient PAT01

  # one patient, one variant only
  python step3_track_progress.py --dataset D:\NahidW\Dataset\face_burn_dataset --patient PAT01 --variant A
"""
...
def centroid_dist(c1, c2) -> float:
    """Works for both 2D [x,y] and 3D [x,y,z] centroids -- but both inputs
    must have the SAME dimensionality (both 2D or both 3D). Raises
    ValueError otherwise; callers should use comparable_centroids() first
    to guarantee that, since a patient's scans can be a mix of aligned
    (3D) and unaligned (2D) when some scans lack an alignment file."""
    a, b = np.array(c1, dtype=float), np.array(c2, dtype=float)
    if a.shape != b.shape:
        raise ValueError(
            f"centroid_dist: mismatched dimensions {a.shape} vs {b.shape} "
            f"-- one side is 3D-aligned and the other isn't; use "
            f"comparable_centroids() to pick a common representation first."
        )
    return float(np.linalg.norm(a - b))


def comparable_centroids(pr: dict, cr: dict):
    """Return a (c1, c2) pair guaranteed to share dimensionality, so
    centroid_dist() never sees a 3D vs 2D mismatch. Prefers the 3D
    aligned centroid when BOTH regions have one; otherwise falls back
    to the 2D pixel centroid, which always exists."""
    if pr.get("aligned_centroid_xyz") is not None and cr.get("aligned_centroid_xyz") is not None:
        return pr["aligned_centroid_xyz"], cr["aligned_centroid_xyz"]
    return pr["centroid_xy"], cr["centroid_xy"]
...
def match_regions(prev_regions: list, curr_regions: list):
    ...
    Centroid comparisons use comparable_centroids(), which picks 3D (mm)
    when BOTH regions in a pair have aligned data, or 2D (pixels)
    otherwise -- this handles patients whose scans are a mix of aligned
    and unaligned (e.g. some timepoints missing an alignment file), where
    a straight 3D-vs-2D distance would be meaningless. MAX_CENTROID_DIST
    is in mm for aligned pairs (where 1mm ≈ ~1px at typical scanner
    resolution) so the same threshold works reasonably for both cases.
    """
    ...
    for i, pr in enumerate(prev_regions):
        ...
        for j, cr in enumerate(curr_regions):
            if j in used_curr:
                continue
            d = centroid_dist(*comparable_centroids(pr, cr))
...
def run_for_patient(dataset_dir: Path, patient: str, variant_filter: str = None, out_dir: Path = None) -> int:
    """Returns the number of variant-tracking JSONs written for this patient."""
    ...
    if not groups:
        print(f"  No matching variant group(s) for {patient}")
        return 0
...
def run_for_all_patients(dataset_dir: Path, variant_filter: str = None):
    """Run run_for_patient() for every patient subfolder found directly under dataset_dir."""
    patient_dirs = sorted([p for p in dataset_dir.iterdir() if p.is_dir()])
    if not patient_dirs:
        print(f"  No patient folders found under {dataset_dir}")
        return

    print(f"  Found {len(patient_dirs)} patient folder(s) under {dataset_dir}\n")

    total_written = 0
    for pd in patient_dirs:
        patient = pd.name
        print(f"=== Patient: {patient} ===")
        total_written += run_for_patient(dataset_dir, patient, variant_filter)
        print()

    print(f"  ALL PATIENTS DONE — {total_written} variant-tracking file(s) written")
...
def main():
    ...
    p.add_argument("--patient", default=None,
                    help="Patient ID, e.g. PAT01. Omit to process ALL patients found under --dataset.")
    ...
    if args.patient:
        ...
        run_for_all_patients(Path(args.dataset), args.variant)
